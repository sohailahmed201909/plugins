...
echo $query =   "SELECT * from $wpdb->prefix" . "process_list order by id desc";
...
<?php
  ...
  if(isset($_REQUEST['start_process_id'])){
	  $import_id = $_REQUEST['start_process_id'];
	  //$PID = shell_exec("nohup php /opt/bitnami/apps/wordpress/hit/run.php $import_id < /dev/null &");	  
	  //$wpdb->get_var($wpdb->prepare("UPDATE wp_process_list SET process_id = $PID WHERE id = $_REQUEST['start_process_id']"));
  }
  
  if(isset($_REQUEST['cancle_process_id'])){
	  $cancel_id = $_REQUEST['cancle_process_id'];
	  $get_process_id = $wpdb->get_var($wpdb->prepare("SELECT process_id FROM wp_process_list WHERE id = $cancel_id"));
	  //shell_exec('kill '.$get_process_id.' 2>&1',$status);
	  //$get_status = implode("\r\n",$status);
  }
